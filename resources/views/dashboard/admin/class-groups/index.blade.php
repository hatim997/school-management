                ...
                <table class="datatables-users table border-top custom-datatables">
                    <thead>
                        <tr>
                            ...
                            @canany(['view class groups','view class group schedules'])<th>{{ __('Action') }}</th>@endcan
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($classGroups as $index => $classGroup)
                            <tr>
                                ...
                                @canany(['view class groups', 'view class group schedules'])
                                    <td class="d-flex">
                                        @canany(['view class groups'])
                                            <span class="text-nowrap">
                                                <a href="{{ route('dashboard.admin-class-groups.show', $classGroup->id) }}"
                                        ...
                                        @endcan
                                        @canany(['view class group schedules'])
                                            <span class="text-nowrap">
                                                <a href="{{ route('dashboard.class-group-schedules.index', $classGroup->id) }}"
                                                    class="btn btn-icon btn-text-primary waves-effect waves-light rounded-pill me-1 edit-order-btn"
                                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                                    title="{{ __('View Class Group Schedules') }}">
